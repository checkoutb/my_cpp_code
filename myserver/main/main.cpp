...
void onRequest(const HttpRequest& req, HttpResponse* resp)
{
    ...
    if (req.getPath() == "/")
    {
        resp->setStatusCode(HttpResponse::k200_ok);
        resp->setStatusMsg("OK");
        resp->setContentType("text/html");
        resp->addHeader("Server", "Muduo_zzz");
        std::string now = Timestamp::now().format();
    ...
    {
        resp->setStatusCode(HttpResponse::k404_notFound);
        resp->setStatusMsg("Not Found");
        resp->setCloseConnection(true);
    }
}

// 找了半天，最后用github的搜索功能才找到main在哪里。
